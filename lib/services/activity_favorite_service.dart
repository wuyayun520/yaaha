...
class ActivityFavoriteService {
  static const String _favoritesKey = 'favorite_activities';
  ...
  // 添加收藏
  static Future<bool> addFavorite(Map<String, dynamic> activity) async {
    try {
      final prefs = await SharedPreferences.getInstance();
      ...
      currentFavorites.add(activityWithTimestamp);
  ...
  // 移除收藏
  static Future<bool> removeFavorite(Map<String, dynamic> activity) async {
    try {
      final prefs = await SharedPreferences.getInstance();
      final currentFavorites = await getFavorites();
  ...
  // 检查是否已收藏
  static Future<bool> isFavorite(Map<String, dynamic> activity) async {
    try {
